...
abstract class REST_Controller extends CI_Controller {
	...
	function log_request($file_name = 'log_rapipago.log'){
		$this->load->helper('file');
		$getParams = [];
		$getParams['uri'] = $this->uri->uri_string();
		$getParams['method'] = $this->request->method;
		$getParams['params'] = $this->_args ? ($this->config->item('rest_logs_json_params') === TRUE ? json_encode($this->_args) : serialize($this->_args)) : NULL;
		$getParams['api_key'] = isset($this->rest->key) ? $this->rest->key : '';
		$getParams['ip_address'] = $this->input->ip_address();
		$getParams['time'] = time();

		$data  = "#-- INIT POST " . date("Y-m-d H:i:s") . " --# \n";
		foreach ($getParams as $key => $value) {
			$data .= "{$key}: ".trim($value). "\n";
		}
		$data .= "#--END POST" . "--# \n";

		$target = __DIR__ . '/../logs/' . $file_name;
		$handle = fopen($target, 'a') or die('Cannot open file:  '.$target);
		fwrite($handle, $data);
		fclose($handle);
	}
}
